...
Function Get-AllEntraRolesWithMembers {
    [CmdletBinding()] Param (
        [Parameter(Mandatory = $True)]
        [String]$Token
    )

    # Get all Entra ID roles
    $URI        = 'https://graph.microsoft.com/v1.0/directoryRoles'
    $Request    = Invoke-RestMethod `
                    -Headers @{Authorization = "Bearer $($Token)"} `
                    -URI $URI `
    ...
    # Iterate through each role and get its members
    foreach ($Role in $EntraIDRoles) {
        ...
        try {
            $MembersRequest = Invoke-RestMethod `
                -Headers @{Authorization = "Bearer $($Token)"} `
                -URI $MembersURI `
                -Method GET

            $Members = $MembersRequest.value.userPrincipalName
        }
        catch {
            Write-Error "Failed to retrieve members for role $($Role.displayName)"
            $Members = @()
        }

        # Create a custom object with role and members information
        ...
        $RolesWithMembers += $RoleObject
    }

    return $RolesWithMembers
}

$RolesAndMembers = Get-AllEntraRolesWithMembers -Token $Token
$RolesAndMembers | Format-List
...
Function Get-AllEntraAppsWithOwners {
    <#
    .SYNOPSIS
        Retrieves all Entra application registration objects with owner information using the MS Graph API
    ...
    .DESCRIPTION
        Retrieves all Entra application registration objects and their assigned owners using the MS Graph API
        Each app object will include an array of owners with displayName, userPrincipalName, and mail properties

    .PARAMETER Token
        The MS Graph-scoped JWT for the user with read access to Entra apps and owners

    .PARAMETER ShowProgress
        Switch to display progress during enumeration

    .PARAMETER IncludeOwners
        Switch to include owner information for each app (default: $True)
    ...
    Description
    -----------
    Uses the JWT in the $Token variable to list all apps with owner information and put them into the $Apps variable

    .EXAMPLE
    C:\PS> $Apps = Get-AllEntraApps -Token $Token -ShowProgress -IncludeOwners $False

    Description
    -----------
    Lists all apps without fetching owner information (faster)
    ...
    # If IncludeOwners is enabled, fetch owner information for each app
    If ($IncludeOwners) {
        If ($ShowProgress) {
            Write-Progress -Activity "Enumerating Applications" -Status "Fetching owner information..." -Id 1
        }

        $AppCount = $AppObjects.Count
        $CurrentAppCount = 0
        ...
        foreach ($App in $AppObjects) {
            ...
            If ($ShowProgress) {
                $PercentComplete = ([Int32](($CurrentAppCount/$AppCount)*100))
                Write-Progress -Activity "Enumerating Applications" `
                    -Status "Fetching owners: $($App.displayName) [$($CurrentAppCount) of $($AppCount)]" `
                    -PercentComplete $PercentComplete `
                    -Id 1
            }

            # Get owners for this app
            $OwnerURI = "https://graph.microsoft.com/beta/applications/$($App.id)/owners/"
            $OwnerResults = $null
            $AppOwnerObjects = @()

            do {
                try {
                    $OwnerResults = Invoke-RestMethod `
                        -Headers @{
                            Authorization = "Bearer $($Token)"
                            ConsistencyLevel = "eventual"
                        } `
                    ...
                    if ($OwnerResults.value) {
                        $AppOwnerObjects += $OwnerResults.value.userPrincipalName
                    } elseif ($OwnerResults -is [object] -and $OwnerResults.id) {
                        $AppOwnerObjects += $OwnerResults
                    }

                    $OwnerURI = $OwnerResults.'@odata.nextlink'
                }
                catch {
                    Write-Warning "Failed to get owners for app $($App.displayName): $_"
                    $OwnerURI = $null
                }
            } until (!($OwnerURI))

            # Add owners information to the app object
            $App | Add-Member -NotePropertyName "owners" -NotePropertyValue $AppOwnerObjects -Force
            $App | Add-Member -NotePropertyName "ownerCount" -NotePropertyValue $AppOwnerObjects.Count -Force
        }

        If ($ShowProgress) {
            Write-Progress -Activity "Enumerating Applications" -Completed -Id 1
        }
    }
